...
public class BrainLocationStrategy extends BrainStrategy {
  ...
  public void execute(String clientId, CommonMessageModel cmm) {
    System.out.println(
        "[BrainLocationStrategy]execute -> dispose the cmd:"
            + cmm.getCmd().toString()
            + " client_id:"
            + clientId);
  ...
  private void redirectThisMessageToRpi() {
    ...
    try {
      ...
      System.out.println(
          "[BrainLocationStrategy]execute -> send cmd to Rpi:"
              + CommonConfig.SERVER_RPI_CLIENT_KEY);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  private void sendNotificationToIos() {
    if (System.currentTimeMillis() - BrainLocationStrategy.lastPushNotificationToIosTime
        > BrainLocationStrategy.PUSH_NOTIFICATION_TO_IOS_INTERVAL * 1000) {
    ...
    try {
      ...
      String currentLocation =
          result.getString("formatted_address") + " " + result.getString("sematic_description");
      System.out.println(
          "[BrainLocationStrategy]sendNotificationToIos -> address:" + currentLocation);
      // The service is down...
