...
public class BrainHandler extends Thread {
  ...
  private static final int INDEX_CLIENT_ID = 0;
  ...
  public void run() {
    ...
    while (true) {
      try {
        ...
        JSONArray currentStrategyJsonArr =
            strategyJsonObjRoot.getJSONArray(Integer.toString(messageType));
  ...
  private BrainStrategy getStrategy(String className) {
    switch (className) {
      ...
      case "com.ihuxu.hestia.server.library.brain.BrainMobileDataDeviceInfoStrategy":
        return new com.ihuxu.hestia.server.library.brain.BrainMobileDataDeviceInfoStrategy();
